(function (a) {
	...
	a.fn.addTag = function (d, e) {
		...
		this.each(function () {
			...
			if (d != "" && h != true) {
				a("<span>").addClass("taghighlight").append(a("<span>").text(d).append("  "), a("<a>", {
					href: "#",
					title: "Remove tag",
					text: "✖"
				}).click(function () {
	...
	a.fn.removeTag = function (d) {
		...
		this.each(function () {
			...
			a("#" + e + "_tagsinput .taghighlight").remove();
			str = "";
	...
	a.fn.importTags = function (b) {
		id = a(this).attr("id");
		a("#" + id + "_tagsinput .taghighlight").remove();
	...
	a.fn.tagsInput = function (d) {
		...
		this.each(function () {
			...
			if (e.interactive) {
				...
				a(f.fake_input).resetAutosize(e); /*a(f.holder).bind("click",f,function(b){a(b.data.fake_input).focus()});*/
				...
				f.removeWithBackspace && a(f.fake_input).bind("keydown", function (b) {
					if (b.keyCode == 8 && a(this).val() == "") {
						b.preventDefault();
						var c = a(this).closest(".tagsinput").find(".taghighlight:last").text();
